...
#' Gets a graph showing a spending parameter over time.
#'
#' \code{jc_get_spend_cat_graph} Gets ggplot object showing lines representing
...
#'
jc_get_spend_cat_graph <- function(spending_info, spend_param,
...
#'
#' \code{jc_get_spend_cat_stacked} Gets ggplot object showing lines representing
...
#'
jc_get_spend_cat_stacked <- function(spending_info, spend_param,
                              x_info, y_info, categories,
                              colours, line_types, line_width = 1,
                              legend_title = "Category",
                              scale_factor = 1) {
  ...
  spending_info$treat_category <- factor(spending_info$treat,
                                         levels = categories)
  ...
  gr <- ggplot2::ggplot(data=spending_info, aes(x=.data$period,
                                                y=.data$plotvalue)) +
    geom_col(aes(fill = .data$treat_category)) +
    scale_color_manual(values = colours) +
    scale_linetype_manual(values=line_types) +
    scale_x_continuous(limits = c(x_info$min, x_info$max), breaks = x_ticks) +
    scale_y_continuous(limits = c(y_info$min, y_info$max), breaks = y_ticks) +
